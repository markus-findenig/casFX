...
//	}
	...
	/**
	 * Startet den Intervall Output Media Player. Ist die aktuelle ECM Nachricht
...
//		Task<Void> taskStreamOutputPlayer = new Task<Void>() {
//			@Override
//			protected Void call() throws Exception {
		...
		//if (EncryptionController.isStateECMType()) {
			...
			//vlcArgs.add("--ts-csa-ck=0123456789ABCDEF");
			vlcArgs.add("--ts-csa-ck=" + decryptionECM.getEcmCwOdd());
		...
		// else msg is odd = 8000000000000000
			...
			vlcArgs.add("--ts-csa2-ck=" + decryptionECM.getEcmCwEven());
			//vlcArgs.add("--ts-csa2-ck=FEDABC9876543210");
...
//		return taskStreamOutputPlayer;
	...
	private static void runIntervallOutputPlayer(String stream, String[] standardVlcOptions) {
		...
		System.out.println("--- runIntervallPlayerOutput ---");
...
//		controlsOutputPanel.updateVolume(0);
		...
		outputPlayerView.reInitOutputPlayerView(embeddedOutputMediaPlayer, mediaOutputPlayerFactory,
				mediaOutputPlayerComponent, controlsOutputPanel);
		
		//outputPlayerView = new OutputPlayerView(embeddedOutputMediaPlayer, mediaOutputPlayerFactory, mediaOutputPlayerComponent, controlsOutputPanel);

		embeddedOutputMediaPlayer.playMedia(stream);
		
		//embeddedOutputMediaPlayer.play();
		
		//embeddedOutputMediaPlayer.playMedia(stream, standardVlcOptions);
