...
public class TaskTest extends Application {
	...
	public void start(final Stage stage) throws Exception {
		...
		runButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				...
				final Task task = new Task<ObservableList<String>>() {
					...
					// super.done();
					// System.out.println("This is bad, do not do this, this
					// thread " + Thread.currentThread() + " is not the
		...
		final VBox layout = VBoxBuilder.create().spacing(8)
				.children(VBoxBuilder.create().spacing(5).children(
						HBoxBuilder.create().spacing(10).children(runButton, statusLabel).build(), progressBar).build(),
				peopleView).build();
